...
def create_data_token(data, tokenizer):
    """
...
class Preprocessing_Data_Token:
    ...
    def processing_token_data_segment(self,output_file=None):
        ...
        dev = self.data['dev']
        ### fitting tokenize
        self.tokenizer.fit_on_texts(flatten_list(train_data['conversation'])) # need to store
        max_num_tokens = 250
        out_train = create_data_token(train, self.tokenizer)
        out_test = create_data_token(test, self.tokenizer)
        out_dev  = create_data_token(dev, self.tokenizer)
    ...
    def create_pretrain_embedding(self,output_file=None):
        ...
        if output_file:
            pd.to_pickle({'tokenizer': self.tokenizer, 'embedding': glv_embedding_matrix})
